...
exports.THEMES = void 0; var DEFAULT_THEME, themes_default;
var init_themes = __esm({
  "src/config/themes.ts"() {
    ...
    themes_default = { THEMES: exports.THEMES, DEFAULT_THEME };
  }
});
...
var _SecurityUtils; exports.SecurityUtils = void 0; var security_default;
var init_security = __esm({
  "src/security/security.ts"() {
    ...
    _SecurityUtils = class _SecurityUtils {
      ...
      static isPathSafe(filePath, allowedDir) {
        ...
        try {
          ...
          const allowed = path5.resolve(allowedDir);
          return resolved.startsWith(allowed + path5.sep) || resolved === allowed;
        } catch (error) {
          return false;
        }
      }
    ...
    exports.SecurityUtils = _SecurityUtils;
    security_default = exports.SecurityUtils;
  }
});
...
var _ThemeCommand = class _ThemeCommand {
  ...
  async execute(args = []) {
    ...
    if (!exports.THEMES[themeName]) {
      this.logger.log(
        `\u{1F338} Tema "${themeName}" no existe~ Temas disponibles: ${Object.keys(exports.THEMES).join(", ")}`,
        "error"
      );
      return;
    }
  ...
  showAvailableThemes() {
    ...
    console.log("");
    const availableThemes = Object.keys(exports.THEMES);
...
var _DependencyInstaller = class _DependencyInstaller {
  ...
  async isDependencyInstalled(depName, requirement) {
    ...
    if (!method || !method.validateCommand) {
      try {
        await exports.SecurityUtils.execSafe(depName, ["--version"], { stdio: "ignore" });
        return true;
      } catch {
        return false;
      }
  ...
  async validateInstallation(validateCommand) {
    try {
      ...
      await exports.SecurityUtils.execSafe(command || "", args, { stdio: "ignore" });
      return true;
    } catch {
      return false;
    }
  ...
  async executeCommand(command) {
    ...
    try {
      exports.SecurityUtils.execSafe(cmd, args, { stdio: "inherit" });
    } catch (error) {
  ...
  async installNodeDependencies(botPath) {
    ...
    try {
      exports.SecurityUtils.execSafe("npm", optimizedArgs, { cwd: botPath, stdio: "inherit" });
      return true;
    } catch (error) {
      return false;
    }
  ...
  async installPythonDependencies(botPath) {
    ...
    try {
      exports.SecurityUtils.execSafe("python3", optimizedArgs, { cwd: botPath, stdio: "inherit" });
      return true;
    } catch (error) {
      return false;
    }
...
var _CeliaAssistant = class _CeliaAssistant {
  ...
  static checkCriticalPrerequisites() {
    if (!exports.SecurityUtils.validateNodeVersion(NODE_MIN_VERSION)) {
  ...
  showPrerequisiteStatus() {
    const missing = exports.SecurityUtils.checkPrerequisites();
    if (missing.length > 0) {
...
var _FileSystemUtils = class _FileSystemUtils {
  ...
  static removeDirectory(dirPath, system) {
    if (!fs3__namespace.existsSync(dirPath)) return;
    try {
      if (fs3__namespace.rmSync) {
        fs3__namespace.rmSync(dirPath, { recursive: true, force: true });
      } else {
  ...
  static removeDirectoryRecursive(dirPath) {
    if (!fs3__namespace.existsSync(dirPath)) return;
    const files = fs3__namespace.readdirSync(dirPath);
    files.forEach((file) => {
      const filePath = path2__namespace.join(dirPath, file);
      const stat = fs3__namespace.statSync(filePath);
      if (stat.isDirectory()) {
    ...
    fs3__namespace.rmdirSync(dirPath);
  }
  /**
   * System-specific directory removal
   */
  static removeDirectoryWithSystem(dirPath, system) {
    const SecurityUtils2 = (init_security(), __toCommonJS(security_exports));
    try {
  ...
  static ensureDirectory(dirPath) {
    try {
      if (!fs3__namespace.existsSync(dirPath)) {
        fs3__namespace.mkdirSync(dirPath, { recursive: true });
      }
      return true;
    } catch (error) {
      return false;
  ...
  static readFile(filePath, encoding = "utf8") {
    try {
      return fs3__namespace.readFileSync(filePath, encoding);
    } catch (error) {
      return null;
    }
  }
  ...
  static writeFile(filePath, content, encoding = "utf8") {
    try {
      fs3__namespace.writeFileSync(filePath, content, encoding);
      return true;
    } catch (error) {
      return false;
    }
  ...
  static isDirectory(dirPath) {
    try {
      const stat = fs3__namespace.statSync(dirPath);
  ...
  static readDirectory(dirPath) {
    try {
      return fs3__namespace.readdirSync(dirPath);
    } catch (error) {
      return null;
    }
  }
  ...
  static getExtension(filePath) {
    return path2__namespace.extname(filePath);
  }
  /**
   * Get file name without extension
   */
  static getBaseName(filePath, ext) {
    return path2__namespace.basename(filePath, ext);
  }
  /**
   * Get directory name
   */
  ...
  static joinPath(...paths) {
    return path2__namespace.join(...paths);
  }
  /**
   * Normalize path
   */
  static normalizePath(filePath) {
    return path2__namespace.normalize(filePath);
  }
  /**
   * Get absolute path
   */
  static getAbsolutePath(filePath) {
    return path2__namespace.resolve(filePath);
  }
};
__name(_FileSystemUtils, "FileSystemUtils");
var FileSystemUtils = _FileSystemUtils;
